// Generated from Repository.g4 by ANTLR 4.10.1
// jshint ignore: start
import antlr4 from '../../base/vendor/antlr4.js';

// This class defines a complete listener for a parse tree produced by RepositoryParser.
export default class RepositoryListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by RepositoryParser#query.
	enterQuery(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#query.
	exitQuery(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#findQuery.
	enterFindQuery(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#findQuery.
	exitFindQuery(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#updateQuery.
	enterUpdateQuery(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#updateQuery.
	exitUpdateQuery(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#deleteQuery.
	enterDeleteQuery(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#deleteQuery.
	exitDeleteQuery(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#fieldExprList.
	enterFieldExprList(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#fieldExprList.
	exitFieldExprList(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#fieldExpr.
	enterFieldExpr(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#fieldExpr.
	exitFieldExpr(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#identifier.
	enterIdentifier(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#identifier.
	exitIdentifier(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#find.
	enterFind(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#find.
	exitFind(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#limitExpr.
	enterLimitExpr(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#limitExpr.
	exitLimitExpr(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#topExpr.
	enterTopExpr(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#topExpr.
	exitTopExpr(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#skipExpr.
	enterSkipExpr(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#skipExpr.
	exitSkipExpr(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#update.
	enterUpdate(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#update.
	exitUpdate(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#delete.
	enterDelete(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#delete.
	exitDelete(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#keywords.
	enterKeywords(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#keywords.
	exitKeywords(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#literals.
	enterLiterals(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#literals.
	exitLiterals(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#logic.
	enterLogic(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#logic.
	exitLogic(ctx) {
	}


	// Enter a parse tree produced by RepositoryParser#operators.
	enterOperators(ctx) {
	}

	// Exit a parse tree produced by RepositoryParser#operators.
	exitOperators(ctx) {
	}



}